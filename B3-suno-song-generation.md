...
> **범위 경계**: YouTube 검색·자막 취득·가사 정규화는 B1, 확보된 가사의 학습 데이터 변환은 B2, 배경영상 합성 콜백(`video-trigger`/`video-callback`)은 B4에서 다룬다. 본 문서는 "주제 입력 → 가사 생성 → Suno 작곡 → 상태 폴링 → 단어 단위 타임스탬프 → 배경영상 후보 → 아카이브 저장" 사슬만 책임진다.
